import { CompiledTask, CompiledKernel, TaskParams, ResourceType, CompiledRenderPipeline, RenderPipelineParams, CompiledRenderPassInfo } from "./Kernel";
import { elementToInt32Array, int32ArrayToElement } from "../utils/Utils";
import { assert, error } from "../utils/Logging";
import { Field } from "../data/Field";
import { TypeCategory } from "../language/frontend/Type";
import { TextureDimensionality } from "../data/Texture";
...
class Runtime {
    ...
    materializedTrees = [];
    textures = [];
    globalTmpsBuffer = null;
    randStatesBuffer = null;
    pipelineCache = null;
    constructor() { }
    async init() {
    ...
    async launchKernel(kernel, ...args) {
        assert(args.length === kernel.argTypes.length, `Kernel requires ${kernel.argTypes.length} arguments, but ${args.length} is provided`);
        let requiresArgsBuffer = false;
        let requiresRetsBuffer = false;
        let thisArgsBuffer = undefined;
        let thisRetsBufferGPU = undefined;
        let thisRetsBufferCPU = undefined;
        let argsSize = 0;
        let retsSize = 0;
        ...
        let computeEncoder = null;
        let renderEncoder = null;
        let computeState = new EncoderState();
        let renderState = new EncoderState();
        let endCompute = () => {
            if (computeEncoder) {
                computeEncoder.end();
            }
    ...
    recycleArgsBuffer(buffer, size) {
        buffer.destroy();
    }
    createGlobalTmpsBuffer() {
        let size = 65536;
    ...
    createRandStatesBuffer() {
        this.randStatesBuffer = this.device.createBuffer({
            size: 65536 * 4 * 4,
            usage: GPUBufferUsage.STORAGE
        });
    }
    getGPUBindGroupEntries(bindings, argsBuffer, retsBuffer) {
        let entries = [];
    ...
    createGPUTexture(dimensions, dimensionality, format, renderAttachment, requiresStorage, sampleCount) {
        let getDescriptor = () => {
            ...
            if (dimensions.length === 1) {
                ...
                let size = { width: dimensions[0], height: dimensions[1] };
    ...
    createGPUSampler(depth, samplingOptions) {
        let desc = {
    ...
    createGPUCanvasContext(htmlCanvas) {
    ...
    async deviceToHost(field, offsetBytes = 0, sizeBytes = 0) {
        if (sizeBytes === 0) {
    ...
    async hostToDevice(field, hostArray, offsetBytes = 0) {
    ...
    getRootBuffer(treeId) {
        return this.materializedTrees[treeId].rootBuffer;
    }
    async copyImageBitmapToTexture(bitmap, texture) {
        let copySource = {
            source: bitmap
        };
        let copyDest = {
    ...
    async copyImageBitmapsToCubeTexture(bitmaps, texture) {
        for (let i = 0; i < 6; ++i) {
...
class FieldHostSideCopy {
    intArray;
    floatArray;
    constructor(intArray, floatArray) {
...
class IndirectDrawCommand {
    ...
    baseVertex;
    firstInstance;
    constructor(indexCount, instanceCount, firstIndex, baseVertex, firstInstance) {
...
class IndirectPolyfillInfo {
    indirectBuffer;
    indirectCount;
    constructor(indirectBuffer, indirectCount) {
