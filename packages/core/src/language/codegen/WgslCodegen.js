...
import { AtomicOpType, BinaryOpType, BuiltInInputKind, BuiltInOutputKind, FragmentDerivativeDirection, getBuiltinInputComponentCount, getBuiltinInputPrimitiveType, getPointedType, StmtKind, TextureFunctionKind, UnaryOpType } from "../ir/Stmt";
import { IRVisitor } from "../ir/Visitor";
import { OffloadType } from "./Offload";
...
export class CodegenVisitor extends IRVisitor {
    ...
    constructor(runtime, offload, argBytes, retBytes, 
    ...
    previousStageBindings) {
        ...
        this.previousStageBindings = previousStageBindings;
    }
    visitConstStmt(stmt) {
    ...
    visitTextureFunctionStmt(stmt) {
        ...
        switch (stmt.func) {
            ...
            case TextureFunctionKind.SampleLod: {
                ...
                this.emitLet(stmt.getName(), texelTypeName);
                this.body.write(`textureSampleLevel(${textureName}, ${samplerName}, ${coordsExpr}, ${stmt
            ...
            case TextureFunctionKind.SampleCompare: {
                ...
                this.emitLet(stmt.getName(), texelTypeName);
                this.body.write(`textureSampleCompare(${textureName}, ${samplerName}, ${coordsExpr}, ${stmt
                    .getAdditionalOperands()[0]
                    .getName()});\n`);
                break;
            }
    ...
    emitGlobalLoadExpr(stmt, atomic = false) {
        let resourceInfo;
    ...
    emitGlobalStore(stmt, atomic = false) {
        let resourceInfo;
        let ptr = stmt.getPointer();
        if (ptr.getKind() === StmtKind.GlobalPtrStmt) {
            ptr = ptr;
    ...
    visitAtomicOpStmt(stmt) {
        ...
        if (dest.getKind() === StmtKind.GlobalPtrStmt) {
            dest = dest;
            resourceInfo = new ResourceInfo(ResourceType.RootAtomic, dest.field.snodeTree.treeId);
        }
        else {
    ...
    getScalarOrVectorTypeName(dt, numComponents) {
    ...
    getScalarOrVectorExpr(values, typeName) {
    ...
    assembleShader() {
        return (this.globalDecls.getString() +
    ...
    startComputeFunction(blockSizeX) {
...
`;
    ...
    stageInMembers = new Set();
    addStageInMember(name, dt, loc, flat) {
    ...
    stageOutMembers = new Set();
    addStageOutMember(name, dt, loc, flat) {
    ...
    stageOutBuiltinMembers = new Set();
    addStageOutBuiltinMember(name, dt, builtin) {
    ...
    resourceBindings = new ResourceBindingMap();
    getBufferName(buffer) {
        let name = "";
        let binding;
    ...
    declareNewBuffer(buffer, name, binding, elementType, elementCount) {
...
`;
    ...
    getBufferMemberName(buffer) {
        return this.getBufferName(buffer) + ".member";
    }
    getTextureName(textureInfo) {
        ...
        if (!this.resourceBindings.has(textureInfo)) {
            ...
            let templateArgs = "";
            if (isStorageTexture) {
                templateArgs = `<${texture.getGPUTextureFormat()}, write>`;
            }
            else if (isDepth) {
                templateArgs = ``;
            }
    ...
    declareNewTexture(texture, name, typeName, templateArgs, binding) {
        let decl = `
@group(0) @binding(${binding})
var ${name}: ${typeName}${templateArgs};
    ...
    declareNewSampler(sampler, name, typeName, binding) {
        let decl = `
@group(0) @binding(${binding})
var ${name}: ${typeName};
        `;
