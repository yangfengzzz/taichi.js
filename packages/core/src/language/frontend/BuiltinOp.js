import { TypeCategory, ScalarType, VoidType, TypeUtils, PrimitiveType, TypeError } from "./Type";
import { assert, error } from "../../utils/Logging";
import { Value, ValueUtils } from "./Value";
import { AtomicOpType, BinaryOpType, getBinaryOpReturnType, getUnaryOpReturnType, StmtKind, UnaryOpType } from "../ir/Stmt";
class BuiltinOp {
    name;
    arity;
...
class BuiltinNullaryOp extends BuiltinOp {
    resultType;
    func;
    constructor(name, resultType, func) {
        super(name, 0);
...
class BuiltinUnaryOp extends BuiltinOp {
    ...
    op;
    fConst;
    constructor(name, irBuilder, op, fConst = null) {
        super(name, 1);
...
class BuiltinBinaryOp extends BuiltinOp {
    irBuilder;
    allowBroadcastLeftToRight;
    allowBroadcastRightToLeft;
    op;
    fConst;
    constructor(name, irBuilder, allowBroadcastLeftToRight, allowBroadcastRightToLeft, op, fConst = null) {
        super(name, 2);
        this.irBuilder = irBuilder;
        this.allowBroadcastLeftToRight = allowBroadcastLeftToRight;
        this.allowBroadcastRightToLeft = allowBroadcastRightToLeft;
        this.op = op;
        this.fConst = fConst;
    }
    checkType(args) {
        ...
        if (cat0 !== cat1) {
            ...
            if (cat0 === TypeCategory.Scalar) {
                ...
                let numRows0 = type0.getNumRows();
                let numRows1 = type1.getNumRows();
...
class BuiltinCustomOp extends BuiltinOp {
    constructor(name, arity, checkType, apply) {
        super(name, arity);
        this.apply_ = apply;
...
class BuiltinAtomicOp extends BuiltinOp {
    ...
    apply(args) {
        ...
        for (let i = 0; i < args[0].stmts.length; ++i) {
            result.stmts.push(this.irBuilder.create_atomic_op(args[0].stmts[i], args[1].stmts[i], this.op));
        }
        return result;
    }
}
class BuiltinOpFactory {
    static getAtomicOps(irBuilder) {
        let opsMap = new Map();
        let ops = [
    ...
    static getBuiltinOps(irBuilder) {
        let opsMap = new Map();
        let ops = [
        ...
        let store = new BuiltinCustomOp("=", 2, (args) => {
            if (args.length !== 2) {
            ...
            let pointerType = args[0].getType();
            let type1 = args[1].getType();
            ...
            else if (pointerType.getValueType().getCategory() === TypeCategory.Struct &&
            ...
            let pointerType = args[0].getType();
            let type1 = args[1].getType();
            let storeFunc = (ptr, value) => {
                switch (ptr.getKind()) {
                    case StmtKind.GlobalPtrStmt: {
                        return irBuilder.create_global_store(ptr, value);
                    }
                    case StmtKind.GlobalTemporaryStmt: {
                        return irBuilder.create_global_temporary_store(ptr, value);
                    }
                    case StmtKind.AllocaStmt: {
                        return irBuilder.create_local_store(ptr, value);
                    }
                    default: {
        ...
        let load = new BuiltinCustomOp("load", 1, (args) => {
            ...
            for (let i = 0; i < args[0].stmts.length; ++i) {
                ...
                switch (ptr.getKind()) {
                    case StmtKind.GlobalPtrStmt: {
                        result.stmts.push(irBuilder.create_global_load(ptr));
                        break;
                    }
                    case StmtKind.GlobalTemporaryStmt: {
                        result.stmts.push(irBuilder.create_global_temporary_load(ptr));
                        break;
                    }
                    case StmtKind.AllocaStmt: {
                        result.stmts.push(irBuilder.create_local_load(ptr));
                        break;
                    }
                    default: {
        ...
        let comma = new BuiltinCustomOp(",", 2, (args) => {
            if (args.length !== 2) {
        ...
        let concat = new BuiltinCustomOp("concat", 2, (args) => {
            if (args.length !== 2) {
        ...
        let len = new BuiltinCustomOp("len", 1, (args) => {
            ...
            let numRows = args[0].getType().getNumRows();
            return ValueUtils.makeConstantScalar(numRows, irBuilder.get_int32(numRows), PrimitiveType.i32);
        });
        let length = new BuiltinCustomOp("length", 1, (args) => len.checkType(args), (args) => len.apply(args));
        let sum = new BuiltinCustomOp("sum", 1, (args) => {
            if (args.length === 1 && args[0].getType().getCategory() === TypeCategory.Vector) {
                return TypeError.createNoError();
            }
            else {
        ...
        let norm_sqr = new BuiltinCustomOp("norm_sqr", 1, (args) => {
            if (args.length === 1 && args[0].getType().getCategory() === TypeCategory.Vector) {
                return TypeError.createNoError();
            }
            else {
        ...
        let norm = new BuiltinCustomOp("norm", 1, (args) => {
            if (args.length === 1 && args[0].getType().getCategory() === TypeCategory.Vector) {
                return TypeError.createNoError();
            }
            else {
        ...
        let normalized = new BuiltinCustomOp("normalized", 1, (args) => {
            if (args.length === 1 && args[0].getType().getCategory() === TypeCategory.Vector) {
                return TypeError.createNoError();
            }
            else {
        ...
        let cross = new BuiltinCustomOp("cross", 2, (args) => {
            let valid = args.length === 2 &&
                args[0].getType().getCategory() === TypeCategory.Vector &&
                args[1].getType().getCategory() === TypeCategory.Vector &&
                args[0].getType().getNumRows() === 3 &&
                args[1].getType().getNumRows() === 3;
            if (valid) {
                return TypeError.createNoError();
            }
            else {
            ...
            let left = ValueUtils.getVectorComponents(args[0]);
            let right = ValueUtils.getVectorComponents(args[1]);
            let r0 = opsMap
                .get("-")
        ...
        let outer_product = new BuiltinCustomOp("outer_product", 2, (args) => {
            let valid = args.length === 2 &&
                args[0].getType().getCategory() === TypeCategory.Vector &&
                args[1].getType().getCategory() === TypeCategory.Vector;
            if (valid) {
        ...
        let matmul = new BuiltinCustomOp("matmul", 2, (args) => {
            let type0 = args[0].getType();
            let type1 = args[1].getType();
            ...
            let type1 = args[1].getType();
            let mat0 = type0;
            let rows0 = ValueUtils.getMatrixRowVectors(args[0]);
            if (type1.getCategory() === TypeCategory.Matrix) {
                let mat1 = type1;
        ...
        let transpose = new BuiltinCustomOp("transpose", 1, (args) => {
        ...
        let static_ = new BuiltinCustomOp("static", 1, (args) => {
            if (args.length !== 1) {
        ...
        let slice = new BuiltinCustomOp("slice", 3, (args) => {
