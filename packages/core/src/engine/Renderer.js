import { WrapMode } from "../data/Texture";
...
import { classKernel, f32, func, i32, sync, template } from "../api/Kernels";
import { vector } from "../api/Types";
import { canvasTexture } from "../api/Textures";
import { add, clearColor, cross, discard, dot, dpdx, dpdy, getFragCoord, getInstanceIndex, inputFragments, inputVertices, inverse, lookAt, matmul, mergeStructs, ndrange, norm, normalized, ortho, outputColor, outputDepth, outputPosition, outputVertex, range, Static, textureLoad, textureSample, textureSampleCompare, textureSampleLod, textureStore, transpose, useDepth } from "../api/KernelScopeBuiltin";
// import {canvasTexture} from "./ca"
export class Renderer {
    scene;
    htmlCanvas;
    constructor(scene, htmlCanvas) {
        this.scene = scene;
    ...
    batchInfos = [];
    batchesDrawInfos = [];
    batchesDrawInstanceInfos = [];
    batchesDrawInfoBuffers = [];
    batchesDrawInstanceInfoBuffers = [];
    // shadow stuff
    lightShadowMaps = [];
    iblShadowMaps = [];
    geometryOnlyDrawInfos = [];
    geometryOnlyDrawInstanceInfos = [];
    geometryOnlyDrawInfoBuffer;
    geometryOnlyDrawInstanceInfoBuffer;
    // funcs
    uvToDir = () => { };
    dirToUV = () => { };
    tonemap = () => { };
    ...
    async init() {
        ...
        for (let light of this.scene.lights) {
            if (light.castsShadow) {
                ...
                light.shadow.projection = ortho(-0.5 * size[0], 0.5 * size[0], -0.5 * size[1], 0.5 * size[0], 0.0, light.shadow.maxDistance);
    ...
    async initHelperFuncs() {
        ...
        this.ggxDistribution = func((NdotH, alpha) => {
        ...
        this.getLightBrightnessAndDir = func((light, fragPos) => {
        ...
        this.lerp = func((x, y, s) => {
            return x * (1.0 - s) + y * s;
        });
        this.linearTosRGB = func((x) => {
            return Math.pow(x, 1.0 / 2.2);
        });
        this.sRGBToLinear = func((x) => {
            return Math.pow(x, 2.2);
        });
        this.fresnel = func((F0, directions) => {
            return F0 + (1.0 - F0) * (1.0 - Math.abs(directions.HdotV)) ** 5;
        });
        this.evalSpecularBRDF = func((alpha, Fr, directions) => {
        ...
        this.evalBRDF = func((material, normal, lightDir, viewDir) => {
        ...
        this.evalIBL = func((material, normal, viewDir, pos) => {
            let dielectricF0 = [0.04, 0.04, 0.04];
        ...
        this.getNormal = func((normal, normalMap, texCoords, position) => {
            let uvDx = dpdx(texCoords.concat([0.0]));
            let uvDy = dpdy(texCoords.concat([0.0]));
        ...
        this.cosineSampleHemisphere = func((randomSource) => {
            let concentricSampleDisk = (randomSource) => {
                let result = [0.0, 0.0];
                let uOffset = 2.0 * randomSource - 1.0;
        ...
        this.cosineSampleHemispherePdf = func((sampled) => {
            let cosTheta = sampled.z;
            return cosTheta / Math.PI;
        });
    }
    async initKernels() {
        this.zPrePassKernel = classKernel(this, { camera: Camera.getKernelType() }, (camera) => {
            useDepth(this.depthPrePassTexture);
            for (let v of inputVertices(this.sceneData.vertexBuffer, this.sceneData.indexBuffer, Static(this.geometryOnlyDrawInfoBuffer), Static(this.geometryOnlyDrawInfoBuffer.dimensions[0]))) {
        ...
        this.gPrePassKernel = classKernel(this, { camera: Camera.getKernelType() }, (camera) => {
        ...
        this.renderKernel = classKernel(this, { camera: Camera.getKernelType() }, (camera) => {
            useDepth(this.depthTexture);
            ...
            for (let batchID of Static(range(this.batchesDrawInfoBuffers.length))) {
                let getMaterial = (fragment, materialID) => {
                    //@ts-ignore
                ...
                for (let v of inputVertices(this.sceneData.vertexBuffer, this.sceneData.indexBuffer, Static(this.batchesDrawInfoBuffers[batchID]), Static(this.batchesDrawInfoBuffers[batchID].dimensions[0]))) {
                    let instanceIndex = getInstanceIndex();
                    //@ts-ignore
                ...
                for (let f of inputFragments()) {
                    ...
                    let directLighting = [0.0, 0.0, 0.0];
                    ...
                    let environmentLighting = this.evalIBL(material, normal, viewDir, f.position);
        ...
        this.ssaoKernel = classKernel(this, { camera: Camera.getKernelType() }, (camera) => {
            useDepth(this.depthTexture);
            ...
            for (let f of inputFragments()) {
                ...
                let screenSpaceCoords = (clipSpacePos.xy / clipSpacePos.w) * 0.5 + 0.5;
                //@ts-ignore
                let texelIndex = i32(
                //@ts-ignore
                ...
                for (let i of range(numSamples)) {
                    ...
                    let sampledPointScreenSpace = (sampledPointClipSpace.xy / sampledPointClipSpace.w) * 0.5 + 0.5;
                    let texCoords = [sampledPointScreenSpace.x, 1.0 - sampledPointScreenSpace.y];
                    let vis = 1.0;
                    if (sampledPointDepth >=
                        //@ts-ignore
        ...
        this.shadowKernel = classKernel(this, { shadowMap: template(), shadowInfo: template() }, (shadowMap, shadowInfo) => {
            useDepth(shadowMap);
            for (let v of inputVertices(this.sceneData.vertexBuffer, this.sceneData.indexBuffer, Static(this.geometryOnlyDrawInfoBuffer), Static(this.geometryOnlyDrawInfoBuffer.dimensions[0]))) {
                let instanceIndex = getInstanceIndex();
                //@ts-ignore
        ...
        this.combineKernel = classKernel(this, {}, () => {
            ...
            for (let f of inputFragments()) {
                ...
                let color = directLighting - environmentLighting * (1.0 - occlusion);
        ...
        this.presentKernel = classKernel(this, { presentedTexture: template() }, (presentedTexture) => {
    ...
    async initIBL() {
        if (this.scene.ibl) {
            ...
            let prefilterKernel = classKernel(this, () => {
                let kSampleCount = 1024;
                let computeLod = (pdf) => {
                    return ((0.5 *
                        Math.log((6.0 * this.scene.ibl.texture.dimensions[0] * this.scene.ibl.texture.dimensions[0]) /
                            (kSampleCount * pdf))) /
                        Math.log(2.0));
                };
    ...
    async computeDrawBatches() {
        ...
        let textureFreeBatchDrawInfo = [];
        let textureFreeBatchInstanceInfo = [];
        for (let i = 0; i < this.scene.materials.length; ++i) {
            let material = this.scene.materials[i];
            let thisMaterialDrawInfo = [];
            let thisMaterialInstanceInfo = [];
                ...
                if (node.mesh >= 0) {
                    ...
                    for (let prim of mesh.primitives) {
                        if (prim.materialID === i) {
                            let drawInfo = new DrawInfo(prim.indexCount, 1, prim.firstIndex, 0, -1 // firstInstance, we'll fill this later
                            );
        ...
        this.geometryOnlyDrawInstanceInfoBuffer = field(InstanceInfo.getKernelType(), this.geometryOnlyDrawInstanceInfos.length);
