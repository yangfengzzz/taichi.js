...
export class HdrTexture {
    texture;
    exposure;
    constructor(texture, exposure) {
        this.texture = texture;
        this.exposure = exposure;
    }
}
export class HdrLoader {
    static getImgFieldToTextureKernel() {
        if (!this.imgFieldToTexture) {
            this.imgFieldToTexture = kernel({ imgField: template(), imgTexture: template() }, (imgField, imgTexture) => {
    ...
    static imgFieldToTexture = undefined;
    static async loadFromURL(url) {
        let hdr = await fetch(url);
        let rawBuffer = await hdr.arrayBuffer();
        let d8 = new Uint8Array(rawBuffer);
