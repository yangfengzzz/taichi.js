...
let transformerFactory = (context: ts.TransformationContext) => {
  const visit: ts.Visitor = (node) => {
    if (node.kind === ts.SyntaxKind.CallExpression) {
      ...
      if (
        call.expression.getText() === "ti.kernel" ||
        call.expression.getText() === "ti.func" ||
        call.expression.getText() === "ti.classKernel"
      ) {
...
function transformCode(code: string, filename: string, options?: Options) {
  ...
  let printer = ts.createPrinter({ removeComments: false });
