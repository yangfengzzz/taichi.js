...
async function testFieldAccessor(): Promise<boolean> {
  ...
  passed &&=
    ...
    assertEqual(await m1.get([0]), [
      [0, 1],
      [2, 3]
    ]) &&
    assertEqual(await m2.get([1, 1]), [
      [12, 13],
      [14, 15]
    ]);
  ...
  passed &&= assertEqual(await m1.get([1]), [
    [1, 23],
    [4, 56]
  ]);
  ...
  passed &&= assertEqual(await m2.get([1, 1]), [
    [45, 6],
    [78, 9]
  ]);
